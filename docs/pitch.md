...
Harness 长在飞书里，不是飞书遥控网页。给 Linux 服务器用，不是笔记本扫码版。Goal 卡片、一进程多 bot、会变的进度头。
...
The harness lives in Feishu, on a Linux server. Not a remote for the webpage, not a laptop QR setup. Goal cards, multi-bot, a live progress header.

Harness 长在飞书里，给 Linux 服务器用。不是遥控网页，也不是笔记本扫码版。Goal 卡片、一进程多 bot、会变的进度头。
