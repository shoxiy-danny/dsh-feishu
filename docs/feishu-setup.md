...
这不是飞书官方教程。只写这套插件真正用到的最小集。国际版 Lark 一样，域名用 `FEISHU_DOMAIN`。

部署面是 Linux 服务器。首个 bot **不要指望终端扫码**：无头进程没有 TTY，码没处可贴。先在开放平台建好企业自建应用，凭据写进环境变量，再启动 `dsh --profile feishu`。已经通了的 bot 再开第二个，才谈得上把码打回飞书。
